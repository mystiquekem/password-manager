...
The project follows a clean, modular structure where cryptographic logic is separated from business logic and user interface code.

\begin{lstlisting}[language=bash]
password-manager/
|-- src/                # Source code
|   |-- core/           # Business logic (Vault management)
|   |-- crypto/         # Cryptographic primitives and ciphers
|   \-- ui/             # GUI implementation (Tkinter/Custom)
|-- scripts/            # Utility scripts (Benchmarks, Hex samples)
|-- figure/             # Generated diagrams (PNG/SVG)
|-- hex_samples/        # Binary samples for analysis
\-- password-manager.py # Main entry point
\end{lstlisting}

\section{Cryptographic Pipeline}
The system uses a robust cryptographic pipeline for both encryption and decryption. The core principle is the \textbf{Encrypt-then-MAC} construction.
